...
//############################################################ Variables
  ...
  var localDepartureDate = localStorage.getItem("departureDate");
...
//################################################################ Get Local Storage and Set Travel Times
  ...
  $(function() {
    ...
    $("#localDepartureDate").html(localDepartureDate);
    ...
    if (farFlight !== -1) {
      ...
      for (i = 0; i < dayTime.length; i++) {
        ...
        //If the flight duration and departure time mean it is overnight, and if the arrival time is before 10, then
        //it will display the time with a 0 before it, generally for the early morning times e.g. 4:00 - 04:00, as I use 24 hour timings, if overnight and after 10, particularly on far destination flights, it will just show the left over time without the 0
...
//################################################################ Storing and Displaying Selected Flight Info
  ...
  $(function(){
    ...
    $(".flight").on("click", function() {
      ...
      localStorage.setItem("departingDepartureTime", departureTime);
      localStorage.setItem("departingArrivalTime", arrivalTime);

      //The variables containing the flight information are then put into Local Storage

      var localDeparture = localStorage.getItem("departingDepartureTime");
      var localArrival = localStorage.getItem("departingArrivalTime");
      ...
      var totalPrice = use * totalPassengers;
      localStorage.setItem("departingTotalPrice", totalPrice);

      //This variable multiplies the price by the amount of passengers that the user selected previously and stores in Local Storage

      $("#localDeparture").html(localDeparture + " / ");
      $("#localArrival").html(localArrival);
  ...
  $(function() {
    ...
    $("td").on("click", function() {
      ...
      //This variable holds the value of the src attribute of the image element which is the child of the td element
        ...
        localStorage.setItem("departingSeats", seats);
        var chosenSeats = localStorage.getItem("departingSeats");
        ...
        if (seats.length == totalPassengers) {
          $("#chooseReturn").show("slide", {direction: "left"}, 1000);
        }
        else {
          $("#chooseReturn").hide("slide", {direction: "left"}, 1000);
        }
